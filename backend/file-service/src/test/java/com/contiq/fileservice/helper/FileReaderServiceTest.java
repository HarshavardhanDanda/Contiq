...
class FileReaderServiceTest {
    ...
    public static final byte[] byteData = new byte[]{37, 80, 68, 70, 45, 49, 46, 49, 10, 37, -62, -91, -62, -79, -61,
    ...
    @Test
    void testParsePdfContent_success() throws IOException {
        byte[] pdfBytes = byteData;
        String parsedContent = fileReaderService.parsePdfContent(pdfBytes);
        assertNotNull(parsedContent, "Parsed content should not be null");
        assertFalse(parsedContent.isEmpty(), "Parsed content should not be empty");
    }

    @Test
    void testParsePdfContent_corruptedPdf() {
        byte[] corruptedPdfBytes = "Not a real PDF content".getBytes();
        assertThrows(IOException.class, () -> fileReaderService.parsePdfContent(corruptedPdfBytes),
                "Expected to throw IOException when parsing corrupted data");
    }

    @Test
    void testParsePdfContent_nullInput() {
        assertThrows(NullPointerException.class, () -> fileReaderService.parsePdfContent(null),
                "Expected to throw NullPointerException when passing null input");
    }
}
